...
int main(int argc,char** argv)
{
	...
	ev3_set_time_sp( motors->next, 750 );
	ev3_set_time_sp( motors->next->next, 750);
	//ev3_set_ramp_up_sp( motors, 500);
	//ev3_set_ramp_down_sp( motors, 1000);
	
	ev3_command_motor_by_name( motors->next, "run-always");
	ev3_command_motor_by_name( motors->next->next, "run-always");
